...
for f in $SCRIPT_DIR/*.sql; do 
  ...
  mysql -h $HOST -u $DBUSER --password=$DBPWD -D $DB -e "DROP TABLE IF EXISTS $NAME"
  mysql -h $HOST -u $DBUSER --password=$DBPWD -D $DB < $FULLPATH
done
...
for f in $SCRIPT_DIR/*.csv; do 
  ...
  NAME="${FILENAME%.*}"
   ...
   sed "s/__FILE__/$FULLPATH/gi" $SCRIPT_DIR/import.script | \
      ...
      mysql -h $HOST -u $DBUSER --password=$DBPWD -D $DB --local-infile=1
done
